...
class plgContentMultithumb extends JPlugin {
	...
	// public function __construct(& $subject, $config) {
		...
		$this->botmtversion = 'Multithumb 3.2.0';
 ...
 white-space: pre-wrap;
   ...
   float: none;
	...
	// echo "DEBUG M: $row->id, $row->slug, $row->catid <br/>";
		...
		if ($this->is_article && $this->_params->get('enable_thumbs') < 4) {
			if ( preg_match('/{multithumb}/is', $row->$text_field_name)==0 ) {
				if ($this->_params->get('enable_thumbs') ) {
					...
					if ( ( ( $this->_params->get('enable_thumbs')==2  && !in_array($row->catid, $only_cats ) ) ||
    ...
    function image_replacer(&$matches) {
    	...
    	} elseif (preg_match('#\bborder\b:\s*(.*?);#i', $style, $temp)) {
			$border = $temp[1];
...
//			echo "DEBUG cap:".$this->caption."<br/>";
 ...
 */					$rel = $this->rowid;
    		...
    		switch($this->popup_type) {
    			...
    			case 'prettyPhoto':
				case 'greybox':				
					if( $this->_params->get('max_thumbnails') && $this->_params->get('group_images') ) {
...
$img = '<table '.$imgclass.' cellspacing="0" cellpadding="0" '.$style.' '.$align.' '.$hspace.' '.$vspace.' ><tr><td class="'.$this->_params->get('caption_class').'" '.$caption_style.' >'.$this->caption.'</td><td >'.$img.'</td></tr></table>';
						break;
					case "right":
$img = '<table '.$imgclass.' cellspacing="0" cellpadding="0" '.$style.' '.$align.' '.$hspace.' '.$vspace.' ><tr><td >'.$img.'</td><td class="'.$this->_params->get('caption_class').'" '.$caption_style.' >'.$this->caption.'</td></tr></table>';
						break;
					case "top":
$img = '<table '.$imgclass.' cellspacing="0" cellpadding="0" '.$style.' '.$align.' '.$hspace.' '.$vspace.' ><tr><td class="'.$this->_params->get('caption_class').'" '.$caption_style.' >'.$this->caption.'</td></tr><tr><td >'.$img.'</td></tr></table>';
						break;
					case "bottom":
$img = '<table '.$imgclass.' cellspacing="0" cellpadding="0" '.$style.' '.$align.' '.$hspace.' '.$vspace.' ><tr><td >'.$img.'</td></tr><tr><td class="'.$this->_params->get('caption_class').'" '.$caption_style.' >'.$this->caption.'</td></tr></table>';
// $img = '<div style="display:  table;"'.$imgclass.' cellspacing="0" cellpadding="0" '.$style.' '.$align.' '.$hspace.' '.$vspace.'  '.$border.' ><div style="display:table-row; width:100%;"><div style="display:table-column; float:none; width:100%;" >'.$img.'</div></div><div style="display:table-row; width:100%;"><div style="display:table-column; float:none; width:100%;"  class="'.$this->_params->get('caption_class').'" '.$caption_style.' >'.$this->caption.'</div></div></div>';
	...
	function botmt_thumbnail($filename, &$width, &$height, $proportion='bestfit', $bgcolor = 0xFFFFFF, 
		...
		$prefix = '';
		// echo "DEBUG1: $width $height $watermark $filename<br/>";
...

		{
			...
			if ( isset( $this ) && $zoomin ) {
				...
				switch ($this->_params->get('zoomin_position') ) {
				...
				// echo "DEBUG: <img src='".JURI::base(false)."/".$this->_params->get('zoomin_file', 'plugins/content/multithumb/zoomin.png')."'/><br>";
...
';
    ...
    function set_popup_type( &$title, &$alt ) {
    	...
    	if($new_popup_style!==false) {
    		...
    		$alt = preg_replace('/^(mt_none|mt_popup|mt_thumbnail|mt_lightbox|mt_expand|mt_gallery|mt_ignore|mt_nothumb|mt_greybox|mt_highslide|mt_slimbox|mt_prettyPhoto|mt_shadowbox|mt_thickbox):?/i', '', $alt);

    	}
// echo "DEBUG:$this->popup_type<br/>";
